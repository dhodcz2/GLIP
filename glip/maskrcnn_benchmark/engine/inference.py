...
from glip.maskrcnn_benchmark.data.datasets.evaluation import evaluate, im_detect_bbox_aug
from ..utils.comm import is_main_process
from ..utils.comm import all_gather
from ..utils.comm import synchronize
import pdb
from glip.maskrcnn_benchmark.data.datasets.evaluation.flickr.flickr_eval import FlickrEvaluator
from glip.maskrcnn_benchmark.structures.bounding_box import BoxList
import matplotlib.pyplot as plt
import matplotlib.pylab as pylab
from glip.maskrcnn_benchmark.data.datasets.tsv import load_from_yaml_file
...
def inference_default(
    ...
    logger = logging.getLogger("glip.maskrcnn_benchmark.inference")
...
def build_lvis_evaluator(ann_file, fixed_ap=True):
    from glip.maskrcnn_benchmark.data.datasets.evaluation.lvis.lvis import LVIS
    from glip.maskrcnn_benchmark.data.datasets.evaluation.lvis.lvis_eval import LvisEvaluatorFixedAP, LvisEvaluator
...
def inference(
    ...
    if evaluator is not None:
        ...
        print(score)
        import glip.maskrcnn_benchmark.utils.mdetr_dist as dist
        if is_main_process():
            if "flickr" in cfg.DATASETS.TEST[0]:
